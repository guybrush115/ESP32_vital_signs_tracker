...
void mqtt_pub(int session_id,int hr,int spo2, float bt) {
    ...
    vital_signs = "{\"device_id\": \"tracker-001\",\"session_id\": "+String(session_id)+",\"hr\": "+String(hr)+",\"spo2\": "+String(spo2)+",\"bt\":"+String(bt,2)+"}";
    while (!mqttClient.connected()) {
      Serial.println("Connecting to MQTT server...");
      mqttClient.connect(device_id,mqttUser,mqttPassword);
    }
    if (mqttClient.publish(("iot/"+String(device_id)+"/8").c_str(),vital_signs.c_str())) 
      Serial.println("MQTT published successful");
